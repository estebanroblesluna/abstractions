...
public class DatabaseResourceService implements ResourceService {
	...
	public InputStream getContentsOfResource(long applicationId, String path) {
		Resource resource = this.repository.findBy(Resource.class, this.makeSearchRestrictions(applicationId, path));
		if (resource == null) {
			return null;
		}
	...
	public void deleteResource(long applicationId, String path) {
		Resource toDelete = this.repository.findBy(Resource.class, this.makeSearchRestrictions(applicationId, path));
		if (toDelete != null)
			this.repository.delete(Resource.class, toDelete.getId());
	}

	public boolean resourceExists(long applicationId, String path) {
		return this.repository.findBy(Resource.class, this.makeSearchRestrictions(applicationId, path)) != null;
	}

	public long getResourceLastModifiedDate(long applicationId, String path) {
		return this.repository.findBy(Resource.class, this.makeSearchRestrictions(applicationId, path)).getLastModifiedDate().getTime();
	}
