...
public class Meta {
  ...
  public static Library getCommonLibrary() {
    ...
    library.addDefinition(createForEachRouter());
    library.addDefinition(createForEachConnection());
  ...
  public static ElementDefinition createForEachRouter() {
    RouterDefinition definition = new RouterDefinition("FOR_EACH");
    definition.setDisplayName("For each router");
    definition.setClassName("com.abstractions.instance.routing.ForEachRouter");

    definition.addProperty(createNameProperty("New for each router"));
    definition.setRouterEvaluatorImplementation("com.abstractions.runtime.interpreter.ForEachRouterEvaluator");
    definition.setRouterEvaluatorScript(false);

    return definition;
  }

  public static ElementDefinition createForEachConnection() {
    ConnectionDefinition definition = new ConnectionDefinition("FOR_EACH_CONNECTION");
    definition.setDisplayName("For each connection");
    definition.setClassName("com.abstractions.instance.core.ForEachConnection");

    definition.setColor("00BB00");

    definition.setAcceptedSourceMax(1);
    definition.setAcceptedTargetMax(1);
    definition.setAcceptedSourceTypes("FOR_EACH");
    definition.setAcceptedTargetTypes("*");

    definition.addProperty(createNameProperty("New for each connection"));
