...
public class DeploymentService {
	...
	public DeploymentService(GenericRepository repository,
			SnapshotService snapshotService, UserService userService,
			ServerService serverService, CloudFrontService cfService) {
	...
	@Transactional
	public List<Flow> getFlows(long deploymentId) {
		Deployment deployment = this.repository.get(Deployment.class,
				deploymentId);
		return deployment.getSnapshot().getFlows();
	}
	...
	@Transactional
	public void addDeployment(long snapshotId, long userId,
			Collection<Long> servers) {
		ApplicationSnapshot applicationSnapshot = this.snapshotService
	...
	private boolean isCDNAware(ApplicationSnapshot applicationSnapshot) {
		return applicationSnapshot
				.getProperty(MessageUtils.APPLICATION_CLOUDFRONT_ACCESS) != null
				&& applicationSnapshot
						.getProperty(MessageUtils.APPLICATION_CLOUDFRONT_SECRET) != null
				&& !applicationSnapshot.getResources().isEmpty();
	}

	@Transactional
	public List<Deployment> getDeployments(long applicationSnapshotId) {
		List<Deployment> deployments = this.repository.get(Deployment.class,
				"snapshot", applicationSnapshotId);
	...
	@Transactional
	public void addLazyComputedCache(long deploymentId, String contextId,
			String elementId, String memcachedURL, String ttl,
			String keyExpression, String cacheExpressions) {
		Deployment deployment = this.repository.get(Deployment.class,
				deploymentId);

		for (DeploymentToServer toServer : deployment.getToServers()) {
			ServerCommand command = new ServerCommand(
					"ADD_LAZY_COMPUTED_CACHE", toServer);
			command.addArgument(
					"applicationId",
					Long.valueOf(
							deployment.getSnapshot().getApplication().getId())
							.toString());
			command.addArgument("deploymentId", Long.valueOf(deploymentId)
					.toString());
	...
	@Transactional
	public void addLazyAutorefreshableCache(long deploymentId,
			String contextId, String elementId, String memcachedURL,
			String oldCacheEntryInMills, String keyExpression,
		...
		for (DeploymentToServer toServer : deployment.getToServers()) {
			ServerCommand command = new ServerCommand(
					"ADD_LAZY_AUTOREFRESHABLE_CACHE", toServer);
			command.addArgument(
					"applicationId",
					Long.valueOf(
							deployment.getSnapshot().getApplication().getId())
	...
	@Transactional
	public void addProfiler(long deploymentId, String contextId,
			String elementId) {
		Deployment deployment = this.repository.get(Deployment.class,
				deploymentId);
	...
	@Transactional
	public void removeProfiler(long deploymentId, String contextId,
			String elementId) {
		Deployment deployment = this.repository.get(Deployment.class,
				deploymentId);

		for (DeploymentToServer toServer : deployment.getToServers()) {
			ServerCommand command = new ServerCommand("REMOVE_PROFILER",
					toServer);
			command.addArgument(
					"applicationId",
					Long.valueOf(
	...
	public JSONObject getProfilingInfo(long deploymentId, String contextId) {
		...
		String applicationId = Long.valueOf(
				deployment.getSnapshot().getApplication().getId()).toString();
		List<Object[]> datas = this.repository
				.getLastProfilingOf(applicationId);
		...
		try {
			...
			profilingInfo.put("profilingInfo",
					ProfilingInfoJSONMarshaller.toJSON(info));
	...
	@Transactional
	public void addLogger(long deploymentId, String contextId,
			String elementId, String beforeExpression, String afterExpression,
			Boolean isBeforeConditional, Boolean isAfterConditional,
		...
		for (DeploymentToServer toServer : deployment.getToServers()) {
			...
			command.addArgument("isBeforeConditional",
					isBeforeConditional.toString());
			command.addArgument("isAfterConditional",
					isAfterConditional.toString());
			command.addArgument("beforeConditionalExpressionValue",
					beforeConditionalExpressionValue);
			command.addArgument("afterConditionalExpressionValue",
	...
	@Transactional
	public void removeLogger(long deploymentId, String contextId,
			String elementId) {
		Deployment deployment = this.repository.get(Deployment.class,
				deploymentId);
	...
	@Transactional
	public JSONObject getLogger(long deploymentId, String contextId,
			String elementId) {
		Deployment deployment = this.repository.get(Deployment.class,
				deploymentId);
		String applicationId = Long.valueOf(
		...
		JSONArray partialResults = new JSONArray();
		List<String> datas = this.repository.getLastLoggingInfoOf(
	...
	@Transactional
	public InputStream startDeployment(long deploymentId, String serverId,
			String serverKey) {
		Deployment deployment = this.repository.get(Deployment.class,
				deploymentId);
		...
		if (server != null) {
			DeploymentToServer toServer = deployment
					.getToServer(server.getId());
			toServer.setState(DeploymentState.STARTED);
			this.repository.save(toServer);
			InputStream io = null;
			try {
				io = this.snapshotService.getZipFor(deployment.getSnapshot()
						.getId());
			} catch (IOException e) {
	...
	@Transactional
	public void endDeploymentWithErrors(long deploymentId, String serverId,
			String serverKey) {
		this.setStateToServer(deploymentId, serverId, serverKey,
				DeploymentState.FINISH_WITH_ERRORS);
	}

	@Transactional
	public void endDeploymentSuccessfully(long deploymentId, String serverId,
			String serverKey) {
		this.setStateToServer(deploymentId, serverId, serverKey,
				DeploymentState.FINISH_SUCCESSFULLY);
	}

	private void setStateToServer(long deploymentId, String serverId,
			String serverKey, DeploymentState state) {
		Deployment deployment = this.repository.get(Deployment.class,
				deploymentId);
		Server server = this.serverService.getServer(serverId, serverKey);
